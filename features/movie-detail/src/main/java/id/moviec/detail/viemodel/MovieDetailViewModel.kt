package id.moviec.detail.viemodel
...
class MovieDetailViewModel @Inject constructor(
    ...
    private val getSimilarMovie: GetSimilarMovie
): BaseViewModel() {
    ...
    fun fetchVideos(movieId: Int) {
        getVideos.execute(
            ...
            }, GetVideos.Params(Const.API_KEY, movieId)
        )
    }
    ...
    fun fetchReviews(movieId: Int) {
        getReviews.execute(
            ...
            }, GetReviews.Params(Const.API_KEY, movieId)
        )
    }
    ...
    private val _getSimilarMovie = MutableLiveData<ResultState<List<MovieModel>?>>()
    val getSimilarMovieLiveData: LiveData<ResultState<List<MovieModel>?>>  = _getSimilarMovie
    fun fetchSimilarMovie(movieId: Int) {
        getSimilarMovie.execute(
            object : DefaultSubscriber<List<MovieModel>?>() {
                override fun onError(error: ResultState<List<MovieModel>?>) {
                    ...
                    Log.d(TAG, "Error : $message")
                    _getSimilarMovie.value = error
                }

                override fun onSuccess(data: ResultState<List<MovieModel>?>) {
                    val list = (data as ResultState.Success).data
                    Log.d(TAG, "Success Popular Fetched : $list")
                    _getSimilarMovie.value = data
                }
            }, GetSimilarMovie.Params(Const.API_KEY, movieId)
        )
    }
